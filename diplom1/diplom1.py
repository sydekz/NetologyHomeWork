...
if __name__ == '__main__':
    ...
    #Иницилизируем объект работающий с API Yandex диска
    uploader = YaUploader(token_ya)
    #Создаем категорию на Yandex диске
    uploader.create_folder(folder_name)

    vk_client = GetVKPhotos(token_vk, '5.126')
    ...
    if count == -1:
        ...
        print('\nGenerating and saving on Yandex.Disk json file')
        djson = list()
        n = len(dnames)
        for i, data in enumerate(dnames, 0):
            # print(f'i - {i} data {char}')
        ...
        file_path = '/' + folder_name + '/json.json'
        uploader.upload(file_path)
